...
CREATE INDEX idx_survey_is_delete ON survey(is_delete);
...
CREATE INDEX idx_option_survey_id ON "option"(survey_id);
CREATE INDEX idx_answer_survey_id ON answer(survey_id);
...
SELECT setval(pg_get_serial_sequence('survey', 'id'), COALESCE((SELECT MAX(id) FROM survey), 1), true);
...
SELECT setval(pg_get_serial_sequence('answer', 'id'), COALESCE((SELECT MAX(id) FROM answer), 1), true);
